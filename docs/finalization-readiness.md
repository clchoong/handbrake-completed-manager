...
Passing readiness does not itself authorize source recycling. After promotion, [Guarded source recycling](source-recycling.md) is available only through a later, separate confirmation, followed by integrity-gated [forward completion](forward-completion-and-recovery.md). Undo is separately prepared after completion and follows the source-first ordering documented in [Guarded replacement undo](undo-workflow.md).
