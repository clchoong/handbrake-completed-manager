...
Passing readiness does not itself authorize source recycling. After promotion, [Guarded source recycling](source-recycling.md) is available only through a later, separate confirmation, followed by integrity-gated [forward completion](forward-completion-and-recovery.md). Source restoration controls, promoted-final recycling, and undo remain disabled. The checkpoint design and crash-boundary rules are documented in [Finalisation transaction design](finalization-transaction-design.md).
