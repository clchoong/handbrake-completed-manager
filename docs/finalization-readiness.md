...
Passing readiness does not itself authorize source recycling. Authorization comes either from the initial path-specific [one-click replacement](one-click-safe-replacement.md) confirmation or from a later separate confirmation in the detailed recovery controls. Recycling is followed by integrity-gated [forward completion](forward-completion-and-recovery.md). Undo is separately prepared after completion and follows the source-first ordering documented in [Guarded replacement undo](undo-workflow.md).
